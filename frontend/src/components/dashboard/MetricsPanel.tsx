...
function KPICard({ label, value, sub, color = "var(--cat)" }: {
  ...
  return (
    <div style={{
      ...
      <div style={{ fontFamily: "JetBrains Mono", fontSize: "0.58rem", letterSpacing: "0.14em", textTransform: "uppercase", color: "var(--muted)", marginBottom: 3, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{label}</div>
      <div style={{ fontFamily: "JetBrains Mono", fontSize: "1.1rem", fontWeight: 700, color, lineHeight: 1.1, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{value}</div>
      {sub && <div style={{ fontSize: "0.55rem", color: "var(--text2)", marginTop: 2, fontFamily: "JetBrains Mono", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{sub}</div>}
    </div>
  );
}
    ...
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 3 }}>
      <div style={{ position: "relative", width: 44, height: 44 }}>
        ...
        <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
          <span style={{ fontFamily: "JetBrains Mono", fontSize: "0.6rem", fontWeight: 700, color }}>{pct}</span>
        </div>
      </div>
      <span style={{ fontSize: "0.48rem", fontFamily: "JetBrains Mono", color: "var(--muted)", textTransform: "uppercase", letterSpacing: "0.06em", textAlign: "center", lineHeight: 1.2 }}>{label}</span>
    </div>
  );
}
...
function SectionHeader({ label, accent = false }: { label: string; accent?: boolean }) {
  return (
    <div style={{
      fontFamily: "Syncopate", fontSize: "0.56rem", letterSpacing: "0.22em",
...
function DeltaRow({ label, adios, stat, fmt = (v: number) => v.toFixed(1) }: {
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "44px 1fr auto", gap: 6, alignItems: "center", fontSize: "0.65rem", fontFamily: "JetBrains Mono", padding: "4px 0", borderBottom: "1px solid var(--border)" }}>
      ...
      <span style={{ color: better ? "var(--green)" : "var(--ore)", fontWeight: 700, fontSize: "0.62rem", textAlign: "right", minWidth: 40 }}>
        {better ? "+" : ""}{fmt(delta)}
      </span>
    </div>
  );
...
function LogEntry({ e }: { e: { t: number; truck: string | null; r: number | null; c: number | null; status: string } }) {
  ...
  return (
    <div style={{ fontSize: "0.62rem", fontFamily: "JetBrains Mono", padding: "3px 8px 3px 10px", background: "var(--panel)", borderLeft: `2px solid ${color}`, borderRadius: 2, display: "flex", gap: 4, alignItems: "center", overflow: "hidden", whiteSpace: "nowrap" }}>
      ...
      <span style={{ fontSize: "0.52rem", color, letterSpacing: "0.05em", textTransform: "uppercase", flexShrink: 0 }}>{ok ? "OK" : iso ? "REJ" : "SKIP"}</span>
    </div>
  );
}
  ...
  return (
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 3 }}>
        <span style={{ fontSize: "0.58rem", fontFamily: "JetBrains Mono", color: "var(--muted)", textTransform: "uppercase", letterSpacing: "0.1em" }}>Mean Spacing</span>
        <span style={{ fontSize: "0.62rem", fontFamily: "JetBrains Mono", color, fontWeight: 700 }}>{value.toFixed(1)}m</span>
      </div>
      <div style={{ height: 5, background: "var(--border)", borderRadius: 3, overflow: "hidden" }}>
        <div style={{ height: "100%", width: `${pct}%`, background: color, borderRadius: 3, transition: "width 0.6s ease" }} />
      </div>
      <div style={{ display: "flex", justifyContent: "space-between", marginTop: 2 }}>
        <span style={{ fontSize: "0.5rem", color: "var(--muted)", fontFamily: "JetBrains Mono" }}>{value.toFixed(1)}m achieved</span>
        <span style={{ fontSize: "0.5rem", color, fontFamily: "JetBrains Mono" }}>{pct.toFixed(0)}% gap closed</span>
      </div>
    </div>
  );
}
...
export default function MetricsPanel() {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", overflow: "hidden", background: "var(--surface)" }}>
      <div style={{ flex: 1, overflowY: "auto", overflowX: "hidden", padding: "10px 10px 14px", display: "flex", flexDirection: "column", gap: 10 }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "5px 8px", background: isPPO ? "rgba(255,205,17,0.06)" : "rgba(255,107,53,0.06)", border: `1px solid ${isPPO ? "rgba(255,205,17,0.18)" : "rgba(255,107,53,0.18)"}`, borderRadius: 4 }}>
          ...
          <span style={{ fontSize: "0.62rem", fontFamily: "JetBrains Mono", color: isPPO ? "var(--acid)" : "var(--ore)", textTransform: "uppercase", letterSpacing: "0.1em" }}>
            {policyLabel}{result ? "" : " · awaiting run"}
          </span>
        </div>
        {/* BC/PPO mismatch warning */}
        {result && !isPPO && (
          <div style={{ padding: "5px 8px", background: "rgba(255,107,53,0.07)", border: "1px solid rgba(255,107,53,0.25)", borderRadius: 4, fontSize: "0.56rem", fontFamily: "JetBrains Mono", color: "var(--ore)", lineHeight: 1.5 }}>
        ...
        {fleetIntel ? (
          <>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 3 }}>
              {fleetIntel.trucks.map((t) => (
                <div key={t.id} style={{ background: "var(--panel)", border: "1px solid var(--border)", borderRadius: 3, padding: "5px 8px", display: "grid", gridTemplateColumns: "26px 1fr auto", alignItems: "center", gap: 6 }}>
                  <div style={{ width: 26, height: 26, borderRadius: 2, background: "rgba(255,205,17,0.1)", border: "1px solid rgba(255,205,17,0.22)", display: "flex", alignItems: "center", justifyContent: "center", fontFamily: "JetBrains Mono", fontSize: "0.5rem", fontWeight: 700, color: "var(--cat)" }}>{t.id}</div>
                  <div>
                    <div style={{ fontSize: "0.62rem", fontFamily: "JetBrains Mono", color: "var(--text)", lineHeight: 1 }}>{t.profile}</div>
                    <div style={{ fontSize: "0.5rem", fontFamily: "JetBrains Mono", color: "var(--muted)", marginTop: 1 }}>R={t.turning_radius_m}m · {t.max_payload_t}t</div>
                  </div>
                  <div style={{ fontSize: "0.5rem", fontFamily: "JetBrains Mono", color: "var(--muted)", textAlign: "right" }}>
                    <div>corridor</div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 6, padding: "4px 8px", background: "rgba(0,212,255,0.05)", border: "1px solid rgba(0,212,255,0.15)", borderRadius: 3 }}>
              ...
              <span style={{ fontSize: "0.52rem", fontFamily: "JetBrains Mono", color: "var(--cyan)", textTransform: "uppercase", letterSpacing: "0.12em" }}>
                CTDE · {fleetIntel.policy_type.toUpperCase()} · {fleetIntel.trucks.length} agents
              </span>
            </div>
          </>
        ) : (
          <div style={{ padding: "10px 0", textAlign: "center", fontSize: "0.6rem", fontFamily: "JetBrains Mono", color: "var(--muted)" }}>
        ...
        <div style={{ background: "var(--panel)", border: "1px solid rgba(255,205,17,0.22)", borderRadius: 5, padding: "10px 14px", position: "relative", overflow: "hidden" }}>
          ...
          <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between" }}>
            <div>
              <div style={{ fontFamily: "JetBrains Mono", fontSize: "0.58rem", letterSpacing: "0.16em", textTransform: "uppercase", color: "var(--muted)", marginBottom: 2 }}>Pack Efficiency</div>
              <div style={{ fontFamily: "JetBrains Mono", fontSize: "2rem", fontWeight: 700, color: "var(--acid)", lineHeight: 1 }}>
                {summary ? `${summary.packing_efficiency.toFixed(1)}%` : "—"}
              </div>
            </div>
            <div style={{ textAlign: "right" }}>
              <div style={{ fontFamily: "JetBrains Mono", fontSize: "0.58rem", color: "var(--muted)", textTransform: "uppercase", letterSpacing: "0.1em" }}>Mean Spacing</div>
              <div style={{ fontFamily: "JetBrains Mono", fontSize: "1.2rem", fontWeight: 700, color: "var(--cyan)" }}>
              ...
              {(result as any)?.spacing_analysis?.density_improvement_pct != null && (
                <div style={{ fontFamily: "JetBrains Mono", fontSize: "0.48rem", color: "var(--green)", marginTop: 1 }}>
            ...
            <>
              ...
              <div style={{ background: "var(--panel)", border: "1px solid var(--border)", borderRadius: 4, padding: "8px 10px" }}>
                ...
                <div style={{ marginTop: 7, padding: "5px 8px", background: sp.density_improvement_pct > 50 ? "rgba(34,197,94,0.07)" : "rgba(255,205,17,0.07)", border: `1px solid ${sp.density_improvement_pct > 50 ? "rgba(34,197,94,0.18)" : "rgba(255,205,17,0.18)"}`, borderRadius: 3 }}>
                  <div style={{ fontSize: "0.52rem", fontFamily: "JetBrains Mono", color: "var(--muted)", textTransform: "uppercase", letterSpacing: "0.1em" }}>Closing gap to staffed ops</div>
                  ...
                  <div style={{ fontSize: "0.5rem", color: "var(--text2)", fontFamily: "JetBrains Mono" }}>auto {sp.autonomous_baseline_m}m → staffed {sp.staffed_target_m}m target</div>
                </div>
              </div>
            </>
          );
            ...
            <div style={{ height: 64, background: "var(--panel)", border: "1px solid var(--border)", borderRadius: 4, padding: 4, flexShrink: 0 }}>
              <ResponsiveContainer width="100%" height="100%">
                <LineChart data={chartData}>
                  ...
                  <Tooltip
                    contentStyle={{ background: "var(--panel)", border: "1px solid var(--border)", fontFamily: "JetBrains Mono", fontSize: "0.62rem", color: "var(--text)" }}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 3, maxHeight: 160, overflowY: "auto" }}>
          {lastLog.length === 0 ? (
            <div style={{ fontSize: "0.62rem", color: "var(--muted)", fontFamily: "JetBrains Mono", padding: "6px 0" }}>Awaiting simulation…</div>
          ) : (
