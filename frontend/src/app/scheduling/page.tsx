...
function StatChip({ label, value, accent = false }: { label: string; value: string | number; accent?: boolean }) {
  return (
    <div className="adios-panel px-3 py-2 min-w-[96px]">
      <div className="font-mono text-[0.9rem] uppercase tracking-[0.14em]" style={{ color: "var(--muted)" }}>
        {label}
      </div>
      <div className="font-mono text-[1.35rem] font-bold" style={{ color: accent ? "var(--acid)" : "var(--text)" }}>
        {value}
      </div>
    </div>
  );
...
export default function SchedulingPage() {
  ...
  const [nDumps, setNDumps] = useState(dash.nDumps);
  const [seed, setSeed] = useState(dash.seed);
  // Live-sync local overrides with the dashboard's run configuration so a
  // dispatch run reflects whatever the user just configured there, while
  // still allowing a per-replay override afterward (mirrors audit/page.tsx).
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      dash.isoThreshold, dash.minDumpSpacing, dash.weights, dash.zoneMode, dash.useML]);
  ...
  const controlsPanel = (
    <div className="h-full overflow-y-auto p-4 flex flex-col gap-4">
      ...
      <div className="font-mono text-[0.85rem] uppercase tracking-widest" style={{ color: "var(--muted)" }}>
      ...
      <div className="flex flex-col gap-3">
        {[
          ...
          <label key={label} className="flex flex-col gap-1 font-mono text-[1.06rem] uppercase tracking-widest" style={{ color: "var(--text2)" }}>
            {label}
            <input
              type="number"
              min={min}
      ...
      <button
        onClick={load}
        disabled={loading}
        className="w-full rounded py-3 text-[0.98rem] font-syncopate tracking-[0.18em] uppercase font-bold transition-all"
        style={{
    ...
    >
      <div className="flex flex-col h-full overflow-hidden">
        <div className="flex items-center gap-4 px-4 lg:px-5 py-3 border-b" style={{ background: "var(--surface)", borderColor: "var(--border)" }}>
          <span className="font-syncopate text-[0.97rem] tracking-[0.2em] uppercase font-bold" style={{ color: "var(--acid)" }}>
            Dispatch Scheduler
          </span>
          <div className="ml-auto font-mono text-[1.03rem] uppercase tracking-widest" style={{ color: "var(--text2)" }}>
            {loading ? "Loading" : data ? `${data.timeline.length} dispatches` : "Awaiting data"}
          </div>
        </div>

        {error && (
          <div className="px-5 py-2 border-b font-mono text-[0.97rem]" style={{ background: "rgba(255,51,102,0.1)", borderColor: "rgba(255,51,102,0.3)", color: "#FF3366" }}>
        ...
        <div className="flex flex-col flex-1 min-h-0 overflow-hidden">
          <div className="flex items-center justify-between gap-3 px-4 py-2 border-b" style={{ background: "var(--surface)", borderColor: "var(--border)" }}>
            <span className="font-syncopate text-[0.9rem] tracking-[0.18em] uppercase" style={{ color: "var(--muted)" }}>
            ...
            {data && (
              <div className="flex items-center gap-2 overflow-x-auto">
                <button
                  ...
                  className="rounded px-3 py-1 font-mono text-[1.01rem] font-bold"
                ...
                <button
                  ...
                  className="rounded px-4 py-1 font-mono text-[1.06rem] font-bold"
                  style={{
                ...
                <span className="font-mono text-[1.06rem] min-w-[52px]" style={{ color: "var(--acid)" }}>
                  t = {tick}
                </span>
                <input
                  type="range"
          ...
          <div className="px-4 pt-3">
            {selectedEntry ? (
              <div className="dispatch-detail-card rounded px-4 py-3">
                <div className="text-center font-syncopate text-[0.9rem] tracking-[0.2em] uppercase mb-3" style={{ color: "var(--acid)" }}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-6 gap-3 text-center">
                  {[
                    ...
                    <div key={k} className="min-w-0">
                      <div className="font-mono text-[0.85rem] uppercase tracking-[0.1em]" style={{ color: "var(--muted)" }}>{k}</div>
                      <div className="font-mono text-[1.18rem] font-bold truncate" style={{ color: selectedEntry.status === "dumped" ? "var(--acid)" : "var(--ore)" }}>{v}</div>
                    </div>
                  ))}
                </div>
              </div>
            ) : (
              <div className="adios-panel rounded px-4 py-3 text-center font-mono text-[1.06rem]" style={{ color: "var(--text2)" }}>
          ...
          <div className="flex-1 overflow-y-auto px-4 py-3">
            {data ? (
              ...
              <div className="h-48 flex items-center justify-center font-mono text-[0.97rem] tracking-[0.1em]" style={{ color: "var(--muted)" }}>
          ...
          <div className="border-t px-4 py-2 min-h-[42px] flex items-center justify-center text-center" style={{ background: "var(--surface)", borderColor: "var(--border)" }}>
            <span className="font-mono text-[1.06rem]" style={{ color: "var(--muted)" }}>
              Click any Gantt chart block to inspect dispatch event.
            </span>
          </div>
        </div>
