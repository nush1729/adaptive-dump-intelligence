    ...
    <div style={{ background: "#080808", minHeight: "100vh", color: "#FFF", fontFamily: "sans-serif" }}>
      ...
      <div style={{ paddingTop: 56 }}>
        ...
        <section style={{ padding: "48px 64px", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
          <div style={{ fontFamily: "JetBrains Mono", fontSize: "0.7rem", color: "#6B7280", letterSpacing: "0.16em", textTransform: "uppercase", marginBottom: 28 }}>Key Performance Indicators</div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
            {KEY_METRICS.map((m, i) => (
              <motion.div key={m.label} initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35, delay: i * 0.07 }} style={{ padding: "24px 22px", border: `1px solid ${m.color}22`, borderRadius: 4, background: `${m.color}06`, position: "relative", overflow: "hidden" }}>
                ...
                <div style={{ fontFamily: "'Syncopate', sans-serif", fontWeight: 700, fontSize: "0.92rem", letterSpacing: "0.14em", color: "#FFF", textTransform: "uppercase", marginBottom: 8 }}>{m.label}</div>
                <p style={{ fontFamily: "JetBrains Mono", fontSize: "0.73rem", color: "#6B7280", lineHeight: 1.65, margin: 0 }}>{m.desc}</p>
              </motion.div>
            ))}
          </div>
        </section>
        ...
        <section style={{ padding: "48px 64px 64px" }}>
          <div style={{ fontFamily: "JetBrains Mono", fontSize: "0.7rem", color: "#6B7280", letterSpacing: "0.16em", textTransform: "uppercase", marginBottom: 20 }}>ADIOS vs Staffed Operations</div>
          <div style={{ border: "1px solid rgba(255,255,255,0.06)", borderRadius: 4, overflow: "hidden" }}>
            <div style={{ display: "grid", gridTemplateColumns: "2fr 1.5fr 1.5fr 1.5fr", background: "rgba(255,184,0,0.05)", borderBottom: "1px solid rgba(255,184,0,0.12)" }}>
              {["Metric", "Staffed", "ADIOS", "Improvement"].map((h) => (
                <div key={h} style={{ padding: "10px 20px", fontFamily: "JetBrains Mono", fontSize: "0.7rem", color: "#FFB800", letterSpacing: "0.14em", textTransform: "uppercase" }}>{h}</div>
              ))}
            </div>
            {COMPARISON.map((row, i) => (
              <motion.div key={row.metric} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3, delay: i * 0.05 }} style={{ display: "grid", gridTemplateColumns: "2fr 1.5fr 1.5fr 1.5fr", borderBottom: "1px solid rgba(255,255,255,0.04)" }}>
                <div style={{ padding: "12px 20px", fontFamily: "JetBrains Mono", fontSize: "0.78rem", color: "#FFF" }}>{row.metric}</div>
                <div style={{ padding: "12px 20px", fontFamily: "JetBrains Mono", fontSize: "0.78rem", color: "#6B7280" }}>{row.staffed}</div>
                <div style={{ padding: "12px 20px", fontFamily: "JetBrains Mono", fontSize: "0.78rem", color: "#22C55E", fontWeight: 700 }}>{row.adios}</div>
                <div style={{ padding: "12px 20px", fontFamily: "JetBrains Mono", fontSize: "0.78rem", color: "#FFB800", fontWeight: 700 }}>{row.improvement}</div>
              </motion.div>
            ))}
          </div>
        </section>
